...
public class Conveyor extends Block implements Autotiler {
    ...
    public void init() {
        ...
        if (this.junctionReplacement == null) {
            this.junctionReplacement = AMBlocks.encasedJunction;
        }

        if (this.bridgeReplacement == null || !(this.bridgeReplacement instanceof ItemBridge)) {
            this.bridgeReplacement = AMBlocks.encasedBridgeConveyor;
        }
    ...
    public void handlePlacementLine(Seq<BuildPlan> plans) {
        if (this.bridgeReplacement != null) {
            Placement.calculateBridges(plans, (ItemBridge)this.bridgeReplacement, (b) -> {
                return b instanceof am.world.blocks.distribution.EncasedConveyor;
            });
        }
    }
        ...
        if (this.junctionReplacement == null) {
            ...
            Boolf<Point2> cont = (p) -> {
                return plans.contains((o) -> {
                    return o.x == req.x + p.x && o.y == req.y + p.y && (req.block instanceof mindustry.world.blocks.distribution.Conveyor || req.block instanceof Junction);
                });
            };
            return (Block)(cont.get(Geometry.d4(req.rotation)) && cont.get(Geometry.d4(req.rotation - 2)) && req.tile() != null && req.tile().block() instanceof mindustry.world.blocks.distribution.Conveyor && Mathf.mod(req.tile().build.rotation - req.rotation, 2) == 1 ? this.junctionReplacement : this);
        }
    }
    ...
    public class ConveyorBuild extends Building implements ChainedBuilding {
        ...
        @Nullable
        public mindustry.world.blocks.distribution.Conveyor.ConveyorBuild nextc;
        ...
        public void draw() {
            int frame = this.enabled && this.clogHeat <= 0.5F ? (int)(Time.time * mindustry.world.blocks.distribution.Conveyor.this.speed * 8.0F * this.timeScale * this.efficiency % 4.0F) : 0;
            Draw.z(29.5F);

            float wheight;
            for(int ix = 0; ix < 4; ++ix) {
                if ((this.blending & 1 << ix) != 0) {
                    int dir = this.rotation - ix;
                    wheight = ix == 0 ? (float)(this.rotation * 90) : (float)(dir * 90);
                    Draw.rect(mindustry.world.blocks.distribution.Conveyor.this.sliced(mindustry.world.blocks.distribution.Conveyor.this.regions[0][frame], ix != 0 ? SliceMode.bottom : SliceMode.top), this.x + (float)(Geometry.d4x(dir) * 8) * 0.75F, this.y + (float)(Geometry.d4y(dir) * 8) * 0.75F, wheight);
                }
            }

            Draw.z(29.8F);
            Draw.rect(mindustry.world.blocks.distribution.Conveyor.this.regions[this.blendbits][frame], this.x, this.y, (float)(8 * this.blendsclx), (float)(8 * this.blendscly), (float)(this.rotation * 90));
            Draw.z(29.9F);
            float layer = 29.9F;
            ...
            for(int i = 0; i < this.len; ++i) {
                ...
                float ixx = this.x + Tmp.v1.x * this.ys[i] + Tmp.v2.x;
        ...
        public void overwrote(Seq<Building> builds) {
            ...
            if (var3 instanceof mindustry.world.blocks.distribution.Conveyor.ConveyorBuild) {
                mindustry.world.blocks.distribution.Conveyor.ConveyorBuild build = (mindustry.world.blocks.distribution.Conveyor.ConveyorBuild)var3;
        ...
        public void onProximityUpdate() {
            ...
            int[] bits = mindustry.world.blocks.distribution.Conveyor.this.buildBlending(this.tile, this.rotation, (BuildPlan[])null, true);
            this.blendbits = bits[0];
            this.blendsclx = bits[1];
            this.blendscly = bits[2];
            this.blending = bits[4];
            this.next = this.front();
            this.nextc = this.next instanceof mindustry.world.blocks.distribution.Conveyor.ConveyorBuild && this.next.team == this.team ? (mindustry.world.blocks.distribution.Conveyor.ConveyorBuild)this.next : null;
        ...
        public void unitOn(Unit unit) {
            if (!(this.clogHeat > 0.5F) && this.enabled) {
                this.noSleep();
                float mspeed = mindustry.world.blocks.distribution.Conveyor.this.speed * 8.0F * 55.0F;
        ...
        public void updateTile() {
            ...
            if (this.len == 0 && Mathf.equal(this.timeScale, 1.0F)) {
                ...
                float moved = mindustry.world.blocks.distribution.Conveyor.this.speed * this.edelta();
